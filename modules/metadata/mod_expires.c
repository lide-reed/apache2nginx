...
static const char* get_parsing_code(apr_pool_t *p, const char* code)
{
    if( !code ) return NULL;
    ...
    case 'M':
        additional_sec = atoi(&code[1]);
		rv = apn_time_to_string(p, &timestr, additional_sec);
		if(rv != APR_SUCCESS || !timestr){
			apn_warning("apn_time_to_string error.");
			return NULL;
		}
		args = apr_pstrcat(p, "modified +", timestr, NULL); 
		break;
    case 'A':
        additional_sec = atoi(&code[1]);
		...
		args = apr_pstrcat(p, "+", timestr, NULL); 
        break;
    default:
		apn_warning("expires error!");
		break;
    }
...
static void* convert_expires_dir_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    expires_dir_config *dconf = ap_get_module_config(v, &expires_module);
	...
	/** TODO */
    int wildcards = dconf->wildcards;

    apr_table_t *expiresbytype = dconf->expiresbytype;

    /** expires must be config in location block of nginx conf file */
    if( expiresbytype && parms->path ) {
        const apr_array_header_t *t = apr_table_elts(expiresbytype);
        ...
        for (i = 0; i < t->nelts; i++) {
            ...
            if( key && val ){
                ...
                args = get_parsing_code(p, val);
                mod = apn_mod_insert_child(mod, "expires", args);
            }
        }

    }

    int active = dconf->active;
	if (active == ACTIVE_OFF){
		mod = apn_mod_insert_sibling(mod, "expires", "off");
	}

	const char* code = dconf->expiresdefault;
    args = get_parsing_code(p, code);
    if( args ){
		mod = apn_mod_insert_sibling(mod, "expires", args);
        if (active == ACTIVE_OFF){
                apn_mod_add_comment(mod, "expires is disabled here");
        }
    }
...
module AP_MODULE_DECLARE_DATA expires_module =
{
    ...
    convert_expires_dir_config, /* convert per dir config structure */
