                                     ...
                                     char *fname, const char *args)
{
    ...
    if (i < 0
        ...
        if (!strcasecmp(fname, "remote_addr")) {
            ...
            /** in Nginx, we only support these header fields */
            if(strcasecmp(fname, "User-Agent") &&
                    strcasecmp(fname, "Host")     &&
                    strcasecmp(fname, "Referer")  &&
                    strcasecmp(fname, "Cookie") ){
                apn_set_unsupport(cmd, 
                        apr_pstrcat(cmd->pool, "Unsupported param ",
                            fname, " by Nginx", NULL) );
            }
...
static void* convert_setenvif_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    sei_cfg_rec *sconf = ap_get_module_config(v, &setenvif_module);
    ...
    for (i = 0; i < sconf->conditionals->nelts; ++i) {
        ...
        if (b->name != last_name) {
            ...
            switch (b->special_type) {
                ...
                case SPECIAL_REQUEST_METHOD:
                    ...
                    break;
                case SPECIAL_REQUEST_PROTOCOL:
                    val = "request_protocol";
                    break;
                case SPECIAL_NOT:
                    if(!strcasecmp(b->name, "User-Agent")){
                        val = "http_user_agent";
                    }else if(!strcasecmp(b->name, "Host")){
                        val = "host";
                    }else if(!strcasecmp(b->name, "Referer")){
                        val = "http_referer";
                    }else if(!strcasecmp(b->name, "Cookie")){
                        val = "cookie_COOKIE";
                    }
                    break;
            }
        ...
        if( b->features ){
            ...
            if( arr->nelts > 0){
                args = apr_pstrcat(p, "($", val, match_str, 
                        b->regex, "\") {", NULL);
            ...
            for (j = 0; j < arr->nelts; ++j) {
                if (*(elts[j].val) == '!') {
                    args = apr_pstrcat(p, "\"$", elts[j].key, "\" 0", NULL);
                    mod = apn_mod_insert_child(mod, "set", args);
                } else {
                    args = apr_pstrcat(p, "\"$", elts[j].key, "\" ", 
                            elts[j].val, NULL);
                    mod = apn_mod_insert_child(mod, "set", args);
                } /* end of else */
            }
...
module AP_MODULE_DECLARE_DATA setenvif_module =
{
    ...
    convert_setenvif_config,    /* convert per dir config structure */
    convert_setenvif_config     /* convert per server config structure */
};
