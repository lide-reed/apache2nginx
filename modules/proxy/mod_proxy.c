...
static const char *
cmd_ProxyPassInterpolateEnv(cmd_parms *cmd, void *struct_ptr_v, int arg)
{
    apn_set_unsupport(cmd, "No relevant directive in Nginx.");

    return ap_set_flag_slot(cmd, struct_ptr_v, arg);
}
    ...
    add_pass(cmd_parms *cmd, void *dummy, const char *arg, int is_regex)
{
    ...
    if ((flags & PROXYPASS_NOCANON) && (flags & PROXYPASS_INTERPOLATE)) {
        apn_set_unsupport(cmd, "The optional nocanon and interpolate keywords are not supported in Nginx.");
    } else if (flags & PROXYPASS_NOCANON) {
        apn_set_unsupport(cmd, "The optional nocanon keyword is not supported in Nginx.");
    } else if (flags & PROXYPASS_INTERPOLATE) {
        apn_set_unsupport(cmd, "The optional interpolate keyword is not supported in Nginx.");
    }
    ...
    if (r[0] == '!' && r[1] == '\0') {
        apn_set_unsupport(cmd, "The ! directive in ProxyPass is not supported in Nginx."); 
...
static const command_rec proxy_cmds[] =
{
    ...
    AP_INIT_FLAG("ProxyPassInterpolateEnv", cmd_ProxyPassInterpolateEnv,
...
static void *
convert_proxy_dir(apr_pool_t *pool, cmd_parms *parms, ap_conf_vector_t* v)
{
    ...
    /* list the proxy_pass in dir */
    if (parms->path != NULL && conf != NULL) {
        struct proxy_alias *new;
        new = (struct proxy_alias *)conf->aliases->elts;

        for (i = 0; i < conf->aliases->nelts; i++) {
            if (new->is_in_dir == 1 && strcmp(new->fake, parms->path) == 0) {
                //printf("real:%s  fake:%s \n", new->real, new->fake);
                mod = apn_mod_insert_sibling(mod, "proxy_pass", new->real);
            }
    ...
    for (i = 0; i < dconf->raliases->nelts; i++) {
        if (ralias->fake != NULL && ralias->real != NULL) {
            char *args = apr_pstrcat(pool, ralias->fake, " ", ralias->real, NULL);
            mod = apn_mod_insert_sibling(mod, "proxy_redirect", args);
        }
...
convert_proxy_server(apr_pool_t *pool, cmd_parms *parms, ap_conf_vector_t* v)
{
    ...
    proxy_server_conf *conf =
        (proxy_server_conf *) ap_get_module_config(v, &proxy_module);


    proxy_worker *worker;
    worker = (proxy_worker *)conf->workers->elts;
    for (i = 0; i < conf->workers->nelts; i++) {
        //printf("worker: %s %s %s %s \n", worker->name, worker->scheme, worker->hostname, worker->route);
        worker++;
    }
    ...
    for (i = 0; i < conf->balancers->nelts; i++) {
        ...
        char *maxattempts;
        if (balancer->max_attempts_set == 1) {
            maxattempts = apr_itoa(pool, balancer->max_attempts);
            maxattempts = apr_pstrcat(pool, " max_fails=", maxattempts, NULL);
        } else {
            maxattempts = "";
        }
        
        proxy_worker *worker2;
        worker2 = (proxy_worker *)balancer->workers->elts;
        for (j = 0; j < balancer->workers->nelts; j++) {
            //printf("worker2: %s %s %s %s \n", worker2->name, worker2->scheme, worker2->hostname, worker2->route);
            char *port = apr_itoa(pool, worker2->port);
            char *hostname;
            if (strcmp(worker2->hostname, "localhost") == 0) {
                hostname = "127.0.0.1";
            } else {
            ...
            if (worker2->lbfactor > 0) {
                lbfactor = apr_itoa(pool, worker2->lbfactor);
                lbfactor = apr_pstrcat(pool, " weight=", lbfactor, NULL);
            } else {
                lbfactor = "";
            }

            if (worker2->timeout_set == 1) {
                timeout = apr_itoa(pool, worker2->timeout / APR_USEC_PER_SEC);
                timeout = apr_pstrcat(pool, " fail_timeout=", timeout, "s", NULL);
            } else {
                timeout = "";
            }

            args = apr_pstrcat(pool, hostname, ":", port, lbfactor, maxattempts, timeout, NULL);
            
            
            temp_mod = apn_mod_insert_sibling(temp_mod, "server", args);
    ...
    for (i = 0; i < conf->aliases->nelts; i++) {
        ...
        char *real;
        char *loc_args = NULL;
        if (new->real != NULL && strncmp(new->real, "balancer://", strlen("balancer://")) == 0) {
            real = apr_pstrcat(pool,"http://", new->real + strlen("balancer://"), NULL);
        } else {
            real = new->real;
        }

        /* don't convert the proxy_pass in dir. */
        if (new->is_in_dir == 0) {
            if (new->regex != NULL) {
                loc_args = apr_pstrcat(pool, "~ ", new->fake, " {", NULL);
                mod = apn_mod_insert_sibling(mod, "location", loc_args);
            } else {
                loc_args = apr_pstrcat(pool, new->fake, " {", NULL);
                mod = apn_mod_insert_sibling(mod, "location", loc_args);
            }
            mod = apn_mod_insert_child(mod, "proxy_pass", real);
        }
    ...
    /* buffer size */
    if (conf->recv_buffer_size_set == 1) {
        mod = apn_mod_insert_sibling(mod, "proxy_buffering", "on");
        char *buffer_size = apr_itoa(pool, conf->recv_buffer_size);
        mod = apn_mod_insert_sibling(mod, "proxy_buffer_size", buffer_size);
    }

    /* proxy error override */
    if (conf->error_override_set == 1 ) {
        if (conf->error_override == 1) {
            mod = apn_mod_insert_sibling(mod, "proxy_intercept_errors", "on");
        }
    }
    ...
    if (conf->timeout_set == 1) {
        char *timeout = 
            apr_itoa(pool, conf->timeout / APR_USEC_PER_SEC ); /* chage microseconds to seconds. */

        mod = apn_mod_insert_sibling(mod, "proxy_connect_timeout", timeout);
        mod = apn_mod_insert_sibling(mod, "proxy_read_timeout", timeout);
        mod = apn_mod_insert_sibling(mod, "proxy_send_timeout", timeout);
    }

    /* preserve host */
    if (conf->preserve_host_set == 1) {
        if (conf->preserve_host == 1) {
            mod = apn_mod_insert_sibling(mod, "proxy_set_header", "Host $http_host");
        }
    }
...
module AP_MODULE_DECLARE_DATA proxy_module =
{
    ...
    convert_proxy_dir,          /* convert per dir config of Proxy */
    convert_proxy_server        /* convert per server config of Proxy */
};
