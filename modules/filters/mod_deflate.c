                                           ...
                                           const char *arg)
{
	...
	apn_set_unsupport(cmd, "There is no corresponding directive in nginx.");
...
static void* convert_deflate_server_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    deflate_filter_config *sconf = (deflate_filter_config *)
        ap_get_module_config(v, &deflate_module);
    ...
    */
	...
	/*
    int windowSize = sconf->windowSize;
    int memlevel = sconf->memlevel;
    char *note_ratio_name = sconf->note_ratio_name;
    char *note_input_name = sconf->note_input_name;
    char *note_output_name = sconf->note_output_name;
	*/

    int compressionlevel = sconf->compressionlevel;

	/* In apache, default value is -1,
	 * but in nginx, the default value is 1 
	 */
    if (compressionlevel == DEFAULT_COMPRESSION){
		compressionlevel = 1;
	}

    apr_size_t bufferSize = sconf->bufferSize;

    const char *filters = parms->dir_context->output_filters;
    const char *filter;
	int found = 0;
    if (filters) {
        while (*filters && (filter = ap_getword(p, &filters, ';'))) {
            if(filter && !strcmp(filter, deflateFilterName)) {
                mod = apn_mod_insert_sibling(mod, "gzip", "on");
                mod = apn_mod_insert_sibling(mod, "gzip_buffers",
                        apr_itoa(p, bufferSize));
                mod = apn_mod_insert_sibling(mod, "gzip_comp_level",
                        apr_itoa(p, compressionlevel));
				found = 1;
            }
        }
    }
	...
	apr_hash_t *ct_output_filters = parms->dir_context->ct_output_filters;
	if(!ct_output_filters) return mod;

	apr_hash_index_t *hi;
	ap_filter_rec_t ct_filter;
	char **value = apr_palloc(p, sizeof(char*));
	apr_ssize_t klen;
	char *args = NULL;

	for (hi = apr_hash_first(p, ct_output_filters); hi; hi = apr_hash_next(hi)) {
		apr_hash_this(hi, (const void**)&ct_filter, &klen, (void**)&value);
		if (!strcasecmp(*value, deflateFilterName)){
			if(!args) {
				args = apr_pstrcat(p, ct_filter.name, NULL);
			}else {
				args = apr_pstrcat(p, args, " ", ct_filter.name, NULL);
			}
		}
	}
	if(args){
		...
		mod = apn_mod_insert_sibling(mod, "gzip_comp_level",
				apr_itoa(p, compressionlevel));
		mod = apn_mod_insert_sibling(mod, "gzip_types", args);
...
module AP_MODULE_DECLARE_DATA deflate_module = {
    ...
    NULL,                          /* convert per dir config structure */
    convert_deflate_server_config  /* convert per server config structure */
};
