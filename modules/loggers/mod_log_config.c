...
static const char *set_buffered_logs_on(cmd_parms *parms, void *dummy, int flag)
{
    ...
    apn_set_unsupport(parms, "No relevant directive in Nginx.");
 ...
 * @brief The below is used for converting the directive .
 * @author Li Jinhu, beagem@nhn.com
 * @date 2012-04-23
 */

#define mod_log_tolower(c)      (char) ((c >= 'A' && c <= 'Z') ? (c | 0x20) : c)

static char *
conv_to_lowercase_underscore(apr_pool_t *pool, char *arg) 
{
    char *dst = apr_pstrdup(pool, arg);

    char *tmp = dst;
    while (tmp && *tmp != '\0') {
        *tmp = mod_log_tolower(*tmp);
...
static char *
convert_format_string(apr_pool_t *pool, apr_array_header_t *format_array, int *prompt) 
{
    log_format_item *entries;
    log_format_item *it;
    int i;
    char *arg = "";

    /* this var means: 0 - the last item is processed successfully. */
    /*                 1 - the last item can't be converted.        */
    int mark = 0;

    if (prompt == NULL) {
        apn_error("prompt is NULL\n");
    }

    /* initialize prompt. */
    *prompt = 0;

    entries = (log_format_item *)format_array->elts;
    for (i = 0; i < format_array->nelts; i++) {
        it = &entries[i];

        if (it->func == constant_item )  {
            if ( (it->arg != NULL && *it->arg != '\n') && mark == 0) {
                arg = apr_pstrcat(pool, arg, it->arg, NULL);
            }
            if (mark == 1) {
                mark = 0;
            }

        } else if (it->func == log_remote_host ) {
            arg = apr_pstrcat(pool, arg, "$remote_addr", NULL);
        } else if (it->func == log_remote_address ) {
            arg = apr_pstrcat(pool, arg, "$remote_addr", NULL);
        } else if (it->func == log_local_address ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        } else if (it->func == log_remote_logname ) {
            arg = apr_pstrcat(pool, arg, "-", NULL);
        } else if (it->func == log_remote_user ) {
            arg = apr_pstrcat(pool, arg, "$remote_user", NULL);
        } else if (it->func == log_request_time ) {
            arg = apr_pstrcat(pool, arg, "$time_local", NULL);
        } else if (it->func == log_request_file ) {
            arg = apr_pstrcat(pool, arg, "$request_filename", NULL);
        } else if (it->func == clf_log_bytes_sent ) {
            arg = apr_pstrcat(pool, arg, "$body_bytes_sent", NULL);
        } else if (it->func == log_bytes_sent ) {
            arg = apr_pstrcat(pool, arg, "$body_bytes_sent", NULL);
        } else if (it->func == log_header_in ) {
            char *temp_field = conv_to_lowercase_underscore(pool, it->arg);
            arg = apr_pstrcat(pool, arg, "$http_", temp_field, NULL);

        } else if (it->func == log_header_out ) {
            char *temp_field = conv_to_lowercase_underscore(pool, it->arg);
            arg = apr_pstrcat(pool, arg, "$sent_http_",temp_field, NULL);

        } else if (it->func == log_note ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        } else if (it->func == log_env_var ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        } else if (it->func == log_server_name ) {
            arg = apr_pstrcat(pool, arg, "$server_addr", NULL);
        } else if (it->func == log_virtual_host ) {
            arg = apr_pstrcat(pool, arg, "$server_name", NULL);
        } else if (it->func == log_server_port ) {
            arg = apr_pstrcat(pool, arg, "$server_port", NULL);
        } else if (it->func == log_pid_tid ) {
            arg = apr_pstrcat(pool, arg, "$pid", NULL);
        } else if (it->func == log_request_protocol ) {
            arg = apr_pstrcat(pool, arg, "$server_protocol", NULL);
        } else if (it->func == log_request_method ) {
            arg = apr_pstrcat(pool, arg, "$request_method", NULL);
        } else if (it->func == log_request_query ) {
            arg = apr_pstrcat(pool, arg, "$query_string", NULL);
        } else if (it->func == log_connection_status ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        } else if (it->func == log_cookie ) {
            char *temp_field = conv_to_lowercase_underscore(pool, it->arg);
            arg = apr_pstrcat(pool, arg, "$cookie_", temp_field, NULL);

        } else if (it->func == log_requests_on_connection ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        } else if (it->func == log_request_line ) {
            arg = apr_pstrcat(pool, arg, "$request", NULL);
        } else if (it->func == log_request_duration_microseconds ) {
            arg = apr_pstrcat(pool, arg, "$request_time", NULL);
        } else if (it->func == log_request_duration ) {
            arg = apr_pstrcat(pool, arg, "$request_time", NULL);
        } else if (it->func == log_request_uri ) {
            arg = apr_pstrcat(pool, arg, "$request_uri", NULL);
        } else if (it->func == log_status ) {
            arg = apr_pstrcat(pool, arg, "$status", NULL);
        } else if (it->func == log_handler ) {
            mark = 1;
            (prompt != NULL) && (*prompt = 1);
        }
...
#define APACHE_DEFAULT_FORMAT_NAME "default"


static void
puts_log_format_comment(apn_module_t *mod, int need_prompt)
{
    if (need_prompt == 1) {
        apn_mod_add_comment(mod, "Some log item(s) can't be converted, please check.");
    }
 ...
 */
static void *
log_config_convert_in_server(apr_pool_t *pool, cmd_parms *parms, ap_conf_vector_t* v)
{
    apn_module_t         *mod = NULL;
    multi_log_state      *mls = NULL;
    ...
    /* For coverage test */
    constant_item(NULL, NULL);
    log_remote_host(NULL, NULL);
    log_remote_address(NULL, NULL);
    log_local_address(NULL, NULL);
    log_remote_logname(NULL, NULL);
    log_remote_user(NULL, NULL);
    log_request_line(NULL, NULL);
    log_request_file(NULL, NULL);
    log_request_uri(NULL, NULL);
    log_request_method(NULL, NULL);
    log_request_protocol(NULL, NULL);
    log_request_query(NULL, NULL);
    log_status(NULL, NULL);
    log_handler(NULL, NULL);
    clf_log_bytes_sent(NULL, NULL);
    log_bytes_sent(NULL, NULL);
    log_header_in(NULL, NULL);

    log_header_out(NULL, NULL);
    log_note(NULL, NULL);
    log_env_var(NULL, NULL);
    log_cookie(NULL, NULL);
    log_request_time(NULL, NULL);
    log_request_duration(NULL, NULL);
    log_request_duration_microseconds(NULL, NULL);
    log_virtual_host(NULL, NULL);
    log_server_port(NULL, NULL);
    log_server_name(NULL, NULL);
    log_pid_tid(NULL, NULL);
    log_connection_status(NULL, NULL);
    log_requests_on_connection(NULL, NULL);

    /* end of coverage */

    mls = ap_get_module_config(v, &log_config_module);
    if (mls == NULL) {
        return NULL;
    }
    ...
    if (mls->default_format_string) {
        format = apr_table_get(mls->formats, mls->default_format_string);
        if (format) {
            mls->default_format = parse_log_string(pool, format, &dummy);
        }
    }

    if (mls->default_format == NULL) {
        mls->default_format = parse_log_string(pool, DEFAULT_LOG_FORMAT, &dummy);
    }

    if (mls->default_format != NULL) {
        format_string = convert_format_string(pool, mls->default_format, &need_prompt);
        args = apr_pstrcat(pool, APACHE_DEFAULT_FORMAT_NAME, " ", "'", format_string, "'", NULL);

        mod = apn_mod_insert_sibling(mod, "log_format", args);
        puts_log_format_comment(mod, need_prompt);
    }

    t = apr_table_elts(mls->formats);
    apr_table_entry_t *entries;
    apr_array_header_t *format_array;

    int i;
    char *key = NULL;
    char *val = NULL;

    entries = (apr_table_entry_t *)t->elts;
    for (i = 0; i < t->nelts; i++) {
        key = entries[i].key;
        val = entries[i].val;

        /* the aim is to avoid reduplicate "CLF" */
        if (strcmp(key, "CLF") == 0) {
            continue;
        }

        format_array = parse_log_string(pool, val, &dummy);
        if (format_array != NULL) {
            format_string = convert_format_string(pool, format_array, &need_prompt);

            char *key_tmp = key;
            if (key != NULL && strcmp(key, "combined") == 0) {
                key = apr_pstrcat(pool, key, "_from_apache", NULL);
            }

            args = apr_pstrcat(pool, key, " ", "'", format_string, "'",  NULL);
            mod = apn_mod_insert_sibling(mod, "log_format", args);

            if (key_tmp != NULL && strcmp(key_tmp, "combined") == 0) {
                apn_mod_add_comment(mod, "The format name of \"combined\" can't be defined in Nginx conf file, because it's the built-in format name. \nSo use \"combined_from_apache\" as the responding format name in Nginx.\n\nSome log item(s) can't be converted, please check.");
            } else {
                puts_log_format_comment(mod, need_prompt);
            }
        }
    ...
    if (mls->config_logs->nelts) {
        ...
        for (i = 0; i < mls->config_logs->nelts; ++i) {
            ...
            /** two type: format name, format sting. */
            if (cls->format_string) {
                format = apr_table_get(mls->formats, cls->format_string);
                if (format) {
                    /* add access_log. */
                    if (cls->fname != NULL && strcmp(cls->fname, "combined") == 0) {
                        args = apr_pstrcat(pool, cls->fname, "_from_apache", " ", cls->format_string, NULL);
                    } else {
                        args = apr_pstrcat(pool, cls->fname, " ", cls->format_string, NULL);
                    }

                    mod = apn_mod_insert_sibling(mod, "access_log", args);

                } else {
                    /* add log_format, and access_log */
                    if (cls->format != NULL) {
                        static int tmp = 0;
                        format_string = convert_format_string(pool, cls->format, &need_prompt);

                        char *temp_format_name = NULL;
                        temp_format_name = apr_pstrcat(pool, "format_", apr_itoa(pool, tmp), NULL);
                        args = apr_pstrcat(pool, temp_format_name, " ", "'", format_string, "'",  NULL);
                        mod = apn_mod_insert_sibling(mod, "log_format", args);
                        puts_log_format_comment(mod, need_prompt);

                        args = apr_pstrcat(pool, cls->fname, " ", temp_format_name, NULL);
                        mod = apn_mod_insert_sibling(mod, "access_log", args);
                        
                        tmp++;
                    }
                }
            } else { /* cls->format_string == NULL , when using TransferLog directive */
                args = apr_pstrcat(pool, cls->fname, " ", APACHE_DEFAULT_FORMAT_NAME, NULL);
                mod = apn_mod_insert_sibling(mod, "access_log", args);
            }
...
module AP_MODULE_DECLARE_DATA log_config_module =
{
    ...
    NULL,
    log_config_convert_in_server  /* convert per server config structure */
};
