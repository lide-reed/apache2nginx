...
#if APR_HAS_USER
/*
 * Expand match-paths (/~user) through Unix /etc/passwd
 * database information (or other OS-specific database)
 */
static char *expand_matchpaths(request_rec *r, char *uri)
{
    if (uri && *uri == '/' && uri[1] == '~') {
        char *p, *user;
...
static int hook_uri2file(request_rec *r)
{
    ...
    if (rulestatus) {
        ...
        if (flen > 6 && strncmp(r->filename, "proxy:", 6) == 0) {
            ...
            /* expand "/~user" prefix */
#if APR_HAS_USER
            r->filename = expand_matchpaths(r, r->filename);
#endif
...
static char           *temp_var_name = NULL;
static apr_table_t    *var_table = NULL;
...
static void
strlow(char *dst, char *src, size_t n)
{
    while (n) {
        *dst = tolower(*src);
        dst++;
...
static char *
get_lowercase_header(apr_pool_t *pool, char *a1_input)
{
    char *header = strstr(a1_input, "%{HTTP:");
    char *dest = "";
    char *temp = NULL;
    ...
    char *header2 = apr_pstrcat(pool, header, NULL);
    temp = header2;
    while (*temp != '\0') {
        if (*temp != '}') {
            temp++;
    ...
    temp = "";
    temp = apr_pstrcat(pool, temp, header2, NULL);

    dest = apr_palloc(pool, strlen(temp) + 1);
    (void)strlow(dest, temp, strlen(temp));
...
static apr_table_t *
initialize_var_table(apr_pool_t *pool)
{
    struct var_map vars[] = {
    ...
    int i = 0;
    for (i = 0; vars[i].apache_var != NULL; i++) {
        if (vars[i].nginx_var != NULL) {
            (void)apr_table_set(t, vars[i].apache_var, vars[i].nginx_var);
        }
    }
...
static char *
construct_condition_args(apr_pool_t *pool, rewritecond_entry *cond_entry, apr_table_t * var_table)
{
    char *a1_input = cond_entry->input;
    char *a1 = "";
    char *a2 = cond_entry->pattern;


    if (a1_input != NULL) {
        a1 = (char *)apr_table_get(var_table, a1_input);
        if (a1 == NULL) {
            /* %{HTTP:varibale} */
            if (strncmp(a1_input, "%{HTTP:", 7) == 0) {
                char *header = get_lowercase_header(pool, a1_input);
                a1 = apr_pstrcat(pool, "$http_", header, NULL);
            } else {
                a1 = cond_entry->input;
            }
        }
    ...
    char *match_mark = "~";
    if (cond_entry->ptype == CONDPAT_REGEX) {
        if ((cond_entry->flags & CONDFLAG_NOCASE) && (cond_entry->flags & CONDFLAG_NOTMATCH)) {
            match_mark = "!~*";
        } else if (cond_entry->flags & CONDFLAG_NOCASE) {
            match_mark = "~*";
        } else if (cond_entry->flags & CONDFLAG_NOTMATCH) {
            match_mark = "!~";
        } 
    } else if (cond_entry->ptype == CONDPAT_STR_EQ) {
        match_mark = "=";
        if (cond_entry->flags & CONDFLAG_NOTMATCH) {
            match_mark = "!=";
        }
    } else if (cond_entry->ptype == CONDPAT_FILE_EXISTS) {
        match_mark = "-f";
        if (cond_entry->flags & CONDFLAG_NOTMATCH) {
            match_mark = "!-f";
        }
    ...
    } else if (cond_entry->ptype == CONDPAT_FILE_DIR) {
        match_mark = "-d";
        if (cond_entry->flags & CONDFLAG_NOTMATCH) {
            match_mark = "!-d";
        }
        ...
        a1 = "";
        a2 = cond_entry->input;
    } else if (cond_entry->ptype == CONDPAT_FILE_XBIT) {
        match_mark = "-x";
        if (cond_entry->flags & CONDFLAG_NOTMATCH) {
            match_mark = "!-x";
        }
    ...
    } else {   /* other ptype can't be processed */
        return NULL;
    }

    if (strcmp(a1,"") == 0 ) {
        return apr_pstrcat(pool, "( ", match_mark, " \"", a2, "\" ) {",  NULL);
    } else {
        return apr_pstrcat(pool, "( ", a1, " ", match_mark, " \"", a2, "\" ) {",  NULL);
    }
}


static char *
construct_rule_args(apr_pool_t *pool, rewriterule_entry *rule_entry)
{
    char   *pattern = rule_entry->pattern;
    char   *output  = rule_entry->output;
    int     flags   = rule_entry->flags;
    char   *flags_in_ngx = NULL;
    
    if (flags & RULEFLAG_FORCEREDIRECT) {

        if (rule_entry->forced_responsecode == HTTP_MOVED_TEMPORARILY) {
            flags_in_ngx = "redirect";
        } else if (rule_entry->forced_responsecode == HTTP_MOVED_PERMANENTLY) {
            flags_in_ngx = "permanent";
        } else {
            flags_in_ngx = "redirect";
        }

    } else if (flags & RULEFLAG_PROXY) {
        flags_in_ngx = "last";
    } else if (flags & RULEFLAG_LASTRULE) {
        flags_in_ngx = "last";
    } else if (flags & RULEFLAG_NEWROUND) {
        flags_in_ngx = NULL;
    } else if (flags & RULEFLAG_PASSTHROUGH) {
        flags_in_ngx = "last";
    } else {
        flags_in_ngx = NULL;
    }
    
    if (flags_in_ngx == NULL) {
        if ( !(flags & RULEFLAG_QSAPPEND) ) { /* no QSA */
            return apr_pstrcat(pool, "\"", pattern, "\"", " ", output, NULL);
        } else {
            return apr_pstrcat(pool, "\"", pattern, "\"", " ", output, "?", NULL);
        } 
        ...
        if ( !(flags & RULEFLAG_QSAPPEND) ) {
            return apr_pstrcat(pool, "\"", pattern, "\"", " ", output, " ", flags_in_ngx, NULL);
        } else {
            return apr_pstrcat(pool, "\"", pattern, "\"", " ", output, "?", " ", flags_in_ngx, NULL);
        }
    }
...
static char *
generate_1s(apr_pool_t *pool, int n)
{
    char *cond_arg = "";
    int j;
    for (j=0; j < n; j++) {
        cond_arg = apr_pstrcat(pool, cond_arg, "1", NULL);
    }
...
static char *
generate_temp_var_name(apr_pool_t *pool) 
{
    char *temp_num = apr_itoa(pool, static_num++);
    temp_var_name = apr_pstrcat(pool, "${rewrite_var_", temp_num, "}", NULL);
...
static apn_module_t *
convert_rewriterule(apr_pool_t *pool, rewriterule_entry *rule_entry, apn_module_t *mod)
{
    rewriterule_entry *p = rule_entry;
    rewritecond_entry *cond_entry = (rewritecond_entry *)p->rewriteconds->elts;
    char *args = NULL;
    
    /* I can't convert to nginx rewrite. */
    if (p->skip > 0 || p->forced_mimetype != NULL || p->forced_handler != NULL) {
        return mod;
    }

    if ( (p->flags & (RULEFLAG_NOSUB | RULEFLAG_STATUS))
        &&( (p->forced_responsecode == HTTP_FORBIDDEN) || (p->forced_responsecode == HTTP_GONE) ) ) {
        
        if (p->rewriteconds->nelts == 0) { /* There is no rewrite condition. */

            args = apr_pstrcat(pool, "( $request_uri ~ \"", rule_entry->pattern, "\" ) {", NULL);
            mod = apn_mod_insert_sibling(mod, "if", args);

            args = apr_itoa(pool, p->forced_responsecode);
            mod = apn_mod_insert_child(mod, "return", args);
        
        } else if (p->rewriteconds->nelts == 1) {

            args = construct_condition_args(pool, cond_entry, var_table);

            if (args != NULL) {

                generate_temp_var_name(pool);
            
                mod = apn_mod_insert_sibling(mod, "if", args);
                args = apr_pstrcat(pool, temp_var_name, " 1", temp_var_name, NULL);
                mod = apn_mod_insert_child(mod, "set", args);
                ...
                args = apr_pstrcat(pool, "( ", temp_var_name, " = ", "11", " ) {", NULL);
                mod = apn_mod_insert_sibling(mod, "if", args);
                args = apr_itoa(pool, p->forced_responsecode);
                mod = apn_mod_insert_child(mod, "return", args);
            } else {
            ...
            if (cond_entry[0].flags & CONDFLAG_ORNEXT ) { /* OR flag */
                int i = 0;
                int num = 0; /* record the number of effective conditions. */

                /* firstly, check whether the condition can be convert to nginx. */
                for (i = 0; i < p->rewriteconds->nelts; i++) {

                    args = construct_condition_args(pool, cond_entry + i, var_table);
                    if (args != NULL) {
                        num++;
                    }
                }
                ...
                if (num >= 1) {
                    ...
                    for (i = 0; i < p->rewriteconds->nelts; i++) {
                        ...
                        if (args != NULL) {
                            mod = apn_mod_insert_sibling(mod, "if", args);
                            args = apr_pstrcat(pool, temp_var_name, " 1", NULL);
                            mod = apn_mod_insert_child(mod, "set", args);
                        }
                    }
                    ...
                    /* char *arg_1s = generate_1s(pool, num + 1); */
                ...
                if (num >= 1) {
                    ...
                    char *arg_1s = generate_1s(pool, p->rewriteconds->nelts + 1);
                    args = apr_pstrcat(pool, "( ", temp_var_name, " = ", arg_1s, " ) {", NULL);
                    mod = apn_mod_insert_sibling(mod, "if", args);
                    args = apr_itoa(pool, p->forced_responsecode);
                    mod = apn_mod_insert_child(mod, "return", args);
        ...
        if (p->rewriteconds->nelts == 0) {
            args = construct_rule_args(pool, p);
            mod = apn_mod_insert_sibling(mod, "rewrite", args);
            ...
            if (args != NULL) {
                ...
                args = construct_rule_args(pool, p);
                mod = apn_mod_insert_child(mod, "rewrite", args);
            } else {
                /* no rewrite rule output. */
            }
            ...
            if (cond_entry[0].flags & CONDFLAG_ORNEXT ) { /* OR flag */
                ...
                int j = 0;
                for (j=0; j < p->rewriteconds->nelts; j++) {
                    char *cond_arg = construct_condition_args(pool, cond_entry+j, var_table);

                    if (cond_arg != NULL) {
                        mod = apn_mod_insert_sibling(mod, "if", cond_arg);
                        mod = apn_mod_insert_child(mod, "rewrite", args);
                    }
                }
                ...
                if (num > 1) {
                    ...
                    for (j=0; j < p->rewriteconds->nelts; j++) {
                        ...
                        if (cond_arg != NULL) {
                            ...
                            char *temp_args = apr_pstrcat(pool, temp_var_name, " 1", temp_var_name, NULL);
                            mod = apn_mod_insert_child(mod, "set", temp_args);
                        }
                    }

                    char *arg_1s = generate_1s(pool, num);
                    args = apr_pstrcat(pool, "( ", temp_var_name, " = ", arg_1s, " ) {", NULL);
                    mod = apn_mod_insert_sibling(mod, "if", args);
                    args = construct_rule_args(pool, p);
                    mod = apn_mod_insert_child(mod, "rewrite", args);
...
static void *
convert_rules(apr_pool_t *pool, apr_array_header_t *rewriterules)
{
    apn_module_t* mod = NULL;
    
    /* var_table is a static var. */
    if (var_table == NULL) {
        var_table = initialize_var_table(pool);
    }
    ...
    for (i=0; i < rewriterules->nelts; i++) {
        ...
        if (p->rewriteconds != NULL) {
            mod = convert_rewriterule(pool, p, mod);
        }
    }
...
static void *
convert_rules_in_server(apr_pool_t *pool, cmd_parms *parms, ap_conf_vector_t* v )
{
    rewrite_server_conf *sconf = ap_get_module_config(v, &rewrite_module);
    ...
    apn_module_t       *mod = NULL;
    apr_array_header_t *rewriterules = sconf->rewriterules;
    if (!rewriterules) {
        return NULL;
    }

    if (sconf->state == ENGINE_DISABLED) {
        return NULL;
    }
    
#ifndef REWRITELOG_DISABLED
    if (sconf->rewritelogfile != NULL) {
        mod = apn_mod_insert_sibling(mod, "rewrite_log", "on");
    }
#endif

    mod = convert_rules(pool, rewriterules);
...
{
    rewrite_perdir_conf *dconf = ap_get_module_config(v, &rewrite_module);
    if (!dconf) {
        return NULL;
    }

    apn_module_t* mod = NULL;
    apr_array_header_t  *rewriterules = dconf->rewriterules;
    if (!rewriterules) {
        return NULL;
    }
...
module AP_MODULE_DECLARE_DATA rewrite_module = {
   ...
   convert_rules_in_server      /* convert per server config structure */
};
