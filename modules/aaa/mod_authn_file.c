...
static void* convert_authn_file_dir_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    authn_file_config_rec *dconf 
        = ap_get_module_config(v, &authn_file_module);
    ...
    const char* file = dconf->pwfile;
    if( file ){
        mod = apn_mod_insert_sibling(mod, "auth_basic_user_file", file);
    }
...
module AP_MODULE_DECLARE_DATA authn_file_module =
{
    ...
    convert_authn_file_dir_config,   /* convert per dir config structure */
