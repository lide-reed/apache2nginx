                                      ...
                                      const char *arg)
{
    ...
    /** except basic, nginx don't support others */
    if( strcasecmp(arg, "file") ){
        apn_set_unsupport(cmd, 
                apr_pstrcat(cmd->pool, 
                    "Unsupported provider ", arg, " by Nginx", NULL));
    }
...
static void* convert_auth_basic_dir_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    auth_basic_config_rec *dconf = 
        ap_get_module_config(v, &auth_basic_module);
    if( !dconf) return NULL;

    apn_module_t* mod = NULL;
    const char *args = NULL;

    /* get the auth_name and auth_type from context of dir */
    const char* auth_name = NULL;
    const char* auth_type = NULL;
    context_per_dir_config *dir_context = parms->dir_context;
    if( dir_context ){
        auth_name = dir_context->ap_auth_name;
        auth_type = dir_context->ap_auth_type;
    }
    if(!auth_name || !auth_type){
        apn_info("either auth_name or auth_type is null");
        return mod;
    }

    /** just care enable and basic authorization */
    if( strcasecmp(auth_type, "basic") ){
        apn_info("It is not the basic authorize");
        return mod;
    }

    int authoritative = dconf->authoritative;
    if(authoritative) {
        args = apr_pstrcat(p, "\"", auth_name, "\"", NULL);
    } else {
        args = "off";
    }

    /** just care the 'file' type */
    authn_provider_list *current_provider = dconf->providers;
    const char *provider_name;
    while (current_provider){
        provider_name = current_provider->provider_name;
        if( !strcasecmp(provider_name, "file") ){
            mod = apn_mod_insert_sibling(mod, "auth_basic", args);
            break;
        }
        current_provider = current_provider->next;
    }
...
module AP_MODULE_DECLARE_DATA auth_basic_module =
{
    ...
    convert_auth_basic_dir_config, /* convert per dir config structure */
