                                      ...
                                      const char *value_, const char* ext)
{
    ...
    *(const char**)((char *)exinfo + offset) = value;
	...
	if (cmd->cmd && cmd->cmd->name){
		const char* name = cmd->cmd->name;
		if (!strcmp(name, "AddHandler")
		   ){
			apn_set_unsupport(cmd, MSG_NO_RELEVANT);

		} else if (!strcmp(name, "AddInputFilter")
				|| !strcmp(name, "AddOutputFilter")
		   ){

            if (strcmp(value, "DEFLATE")) {
                apn_set_unsupport(cmd, 
                        apr_psprintf(cmd->pool, 
                            "Filter %s is not been supported by Nginx", value)
                        );
            }
        }
	}
...
static const char *set_default_language(cmd_parms *cmd, void *mconfig,
                                      const char *arg)
{	
	ap_set_string_slot(cmd, mconfig, arg);
	return NULL;
}

static const char *set_use_path_info_flag(cmd_parms *cmd,
										void *mconfig, int on)
{
    ap_set_flag_slot(cmd, mconfig, on);
	apn_set_unsupport(cmd, MSG_NO_RELEVANT);
	return NULL;
}
                                         ...
                                         const char *ext)
{
	...
	if (cmd->cmd && cmd->cmd->name){
		const char* name = cmd->cmd->name;
		if (!strcmp(name, "RemoveLanguage")
				|| !strcmp(name, "RemoveHandler")
				|| !strcmp(name, "RemoveType")
				|| !strcmp(name, "RemoveEncoding")
				|| !strcmp(name, "RemoveInputFilter")
				|| !strcmp(name, "RemoveOutputFilter")
		   ){
			apn_set_unsupport(cmd, MSG_NO_RELEVANT);
		}
	}
...
static const command_rec mime_cmds[] =
{
    ...
    AP_INIT_TAKE1("DefaultLanguage", set_default_language,
    ...
    AP_INIT_FLAG("ModMimeUsePathInfo", set_use_path_info_flag,
...
static int mime_post_config(apr_pool_t *p, apr_pool_t *plog, apr_pool_t *ptemp, server_rec *s)
{
    ...
    if (!types_confname) {
        ...
        */
        apn_warning("Invalid mime types config path %s",
                     (const char *)ap_get_module_config(s->module_config,
                                                       &mime_module));
        return OK;
    }
    if ((status = ap_pcfg_openfile(&f, ptemp, types_confname))
        ...
        return HTTP_INTERNAL_SERVER_ERROR;
        */
        apn_warning("could not open mime types config file %s.",
                     types_confname);
        return OK;
    }
...
static void* convert_mime_dir_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
	apr_status_t rv;
    apn_module_t* mod = NULL;
    mime_dir_config *dconf = NULL;
    char* args = NULL;
	...
	/** generate the mime.types for nginx */
	static int first = 1;
	if( mime_type_extensions && first ){

        if( apr_hash_count(mime_type_extensions) == 0 ){
            apn_warning("The content of mime.types invalid or null");
            goto go_on;
        }

		/** get the fullname of mime.types according to nginx conf file */
		char* basedir = NULL;
		const char* mime_file_name = NULL;
		if(parms->config_file && parms->config_file->name){
			rv = apr_get_basedir(&basedir, parms->config_file->name, p);
			if(rv != APR_SUCCESS) {
				apn_warning("Failed to get full path of nginx conf");
				goto go_on;
			}
		}
		if( basedir ){
			mime_file_name = 
				apr_pstrcat(p, basedir, "/", NGX_TYPES_CONFIG_FILE, NULL);
		}else{
            apn_warning("Failed to get full path of nginx conf");
			goto go_on;
		}
        ...
        /* test whether the mime_file exists or not */
        char *fullpath = NULL;
        rv = apr_get_realpath(&fullpath, mime_file_name, p);
        if (rv == APR_SUCCESS ){  // file exists.
            apn_error("Config file exists: %s. Exit.\n", fullpath);
            // unlink nginx.conf
            if(parms->config_file && parms->config_file->name){
                apr_file_remove(parms->config_file->name, p);
            }
            exit(1);
        }

		apr_file_t *f;
		rv = apr_file_open(&f, mime_file_name,
				APR_CREATE|APR_WRITE,
				APR_OS_DEFAULT, p);
		if (rv != APR_SUCCESS ){
			apn_warning("could not open mime types config file.");
			goto go_on;
		}

		apr_file_printf(f, "types {\n");
		for (hi = apr_hash_first(p, mime_type_extensions); hi; hi = apr_hash_next(hi)) {
			apr_hash_this(hi, (const void**)&key, &klen, (void**)&value);
			if(value && key){
				apr_file_printf(f, "    %-60s\t\t%s;\n",value, key);
			}
		}
		apr_file_printf(f, "}\n");
...
go_on:
    ...
    dconf = ap_get_module_config(v, &mime_module);
    if( !dconf) return mod;

    char *default_language = dconf->default_language;
    if(default_language){
        mod = apn_mod_insert_sibling(mod, "set $lang", default_language);
    }
   
	/* Map from extension name to extension_info structure */
	apr_hash_t *extension_mappings = dconf->extension_mappings;  
	if(!extension_mappings) return mod;

    if (dconf->remove_mappings) {
        remove_items(p, dconf->remove_mappings, extension_mappings);
    }

    const extension_info *exinfo = NULL;

	for (hi = apr_hash_first(p, extension_mappings); hi; hi = apr_hash_next(hi)) {
		apr_hash_this(hi, (const void**)&key, &klen, (void**)&exinfo);
		if(!exinfo) continue;

		if((value = exinfo->charset_type)){

            args = apr_pstrcat(p, "~* \\.", key, "$ {", NULL);
            mod = apn_mod_insert_sibling(mod, "location", args);

            char* type = exinfo->forced_type;
            if (!type) {
                if (mime_type_extensions){
                    type = apr_hash_get(mime_type_extensions, key,
                            APR_HASH_KEY_STRING);
                }

                if (!type) 
                    type = parms->dir_context->ap_default_type;

                if (!type) type = DEFAULT_CONTENT_TYPE;
            }

            args = apr_pstrcat(p, "'", type, "; charset=", value, "'", NULL);
            mod = apn_mod_insert_child(mod, "add_header Content-Type", args);
        } else if((value = exinfo->forced_type)){
            args = apr_pstrcat(p, "~* \\.", key, "$ {", NULL);
            mod = apn_mod_insert_sibling(mod, "location", args);
            mod = apn_mod_insert_child(mod, "add_header Content-Type", value);
        }

		if((value = exinfo->encoding_type)){
            args = apr_pstrcat(p, "~* \\.", key, "$ {", NULL);
			mod = apn_mod_insert_sibling(mod, "location", args);
			mod = apn_mod_insert_child(mod, "add_header Content-Encoding", value);
		}

		if((value = exinfo->language_type) && key){
            args = apr_pstrcat(p, "~* \\.", key, "$ {", NULL);
			mod = apn_mod_insert_sibling(mod, "location", args);
			mod = apn_mod_insert_child(mod, "add_header Content-Language", value);
		} 
	}
...
module AP_MODULE_DECLARE_DATA mime_module = {
    ...
    convert_mime_dir_config,    /* convert per dir config structure */

	/** types_config is parsed by mime_post_config, so no need to use this hook.*/
