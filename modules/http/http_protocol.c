...
/** added by Reed.*/
/** Get the status number in string according the index.
 */
AP_DECLARE(const char*) ap_status_line_of_response(apr_pool_t *p, int index)
{
    if (index > 0 && index < RESPONSE_CODES)
        return apr_pstrdup(p, status_lines[index]);
