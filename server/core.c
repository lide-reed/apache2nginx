...
static void *create_core_dir_config(apr_pool_t *a, char *dir)
{
    ...
    /* For limited methods, by Reed */
    conf->sec_limit = apr_array_make(a, 2, sizeof(ap_conf_vector_t *));
    conf->limited = -1;
...
static void *create_core_server_config(apr_pool_t *a, server_rec *s)
{
    ...
    /** move the init to init_server_config */
    /*conf->ap_document_root = is_virtual ? NULL : DOCUMENT_LOCATION;*/
    conf->ap_document_root = s->ap_document_root;
...
AP_CORE_DECLARE(void) ap_add_limit_conf(core_dir_config *conf, void *limit_config)
{
                                     ...
                                     const char *arg)
{
    ...
    if (!ap_document_root_check) {
       ...
       cmd->server->ap_document_root = apr_pstrdup(cmd->pool, arg);
       return NULL;
    }
    ...
    if (apr_filepath_merge((char**)&conf->ap_document_root, NULL, arg,
        ...
        if (cmd->server->is_virtual) {
            ...
            apn_warning("DocumentRoot is an invalid directory");
            return NULL;
        }
    }

    /** document root is valid, remember it */
    cmd->local_conf->document_root = 1;
    cmd->server->ap_document_root = apr_pstrdup(cmd->pool, arg);
...
static const char *set_options(cmd_parms *cmd, void *d_, const char *l)
{
    ...
    if ( (opt & OPT_SYM_LINKS ) 
          || (opt & OPT_SYM_OWNER) 
		  || (opt & OPT_EXECCGI)  
		  || (opt & (OPT_MULTI|OPT_EXECCGI) )){
		apn_set_unsupport(cmd, "Only support Includes and Indexes");
    }
...
#else
    ap_conf_vector_t *new_container_conf = ap_create_per_dir_config(cmd->pool);
    core_dir_config *conf = 
        ap_set_config_vectors(cmd->server, new_container_conf, cmd->path,
                                 &core_module, cmd->pool);
    if(!conf){
        apn_error("ap_set_config_vectors errpr!");
    }
    errmsg = ap_walk_config(cmd->directive->first_child, cmd, new_container_conf);

    core_dir_config *c = mconfig;
    c->limited = cmd->limited;
    ap_add_limit_conf(c, new_container_conf);
#endif
    //--------------------
    cmd->limited = -1;
...
/** TODO this function is not support multi platform */
#if 0
static const char* apn_get_merged_path( const char* path, const char* root)
{
    if( !path ) return NULL;
    if( !root ) return path;

    int root_len = strlen(root);
    int len = strlen(path) - root_len;
    if(root[root_len-1] == '/') 
        root_len--;

    if(len >= 0 ){
        if(ap_strstr((char*)path, root)){
...
static const char *dirsection(cmd_parms *cmd, void *mconfig, const char *arg)
{
    ...
    else if (!strcmp(cmd->path, "/") == 0)
    {
        ...
        /** in apache2nginx, we no need to merged path */
        /*
        cmd->path = newpath;
...
/** in apache-2.x, name like xxx_module */
static const char* mpm_modules[] = {
	"mpm_winnt.c",
	"mpm_winnt_module",
	"mpm_netware.c",
	"mpm_netware_module",
...
static const char *start_ifmod(cmd_parms *cmd, void *mconfig, const char *arg)
{
	...
	/** In this case, remember it, not process in future */
	if( not && found ){
		cmd->flag = PARAM_NEEDLESS_PROCESS;
	} else if(!not && !found){
		cmd->flag = PARAM_UNRECOGNIZED;
	}
	...
	*(ap_directive_t **)mconfig = current;
	...
	cmd->flag = PARAM_UNDEFINE;

	if( !found ) {
		retval = "Unrecognized module name, "
			"perhaps it is a module which apache2nginx not supported, "
			"you can correct or comment it and run again";
		cmd->flag = PARAM_UNRECOGNIZED;

		const char** mpm = mpm_modules;
		while(*mpm){
			if( !strcmp(*mpm, arg) ){
				retval = "This module is a Multi-Processing Module of Apache, "
					"which apache2nginx not supported";
				cmd->flag = PARAM_UNSUPPORTED;
				break;
			}
			mpm++;
		}
                                   ...
                                   const char *arg)
{
    ...
    const char* old_ap_server_root = apr_pstrdup( cmd->pool, ap_server_root);
    if ((apr_filepath_merge((char**)&ap_server_root, NULL, arg,
                            APR_FILEPATH_TRUENAME, cmd->pool) != APR_SUCCESS)
        || !ap_is_directory(cmd->pool, ap_server_root)) {

        apn_warning("For localhost conf, ServerRoot must be a valid directory");

        /** In apache2nginx, we don't think it is an error*/
        ap_server_root = old_ap_server_root;
        return NULL;
    }

    /** valid local server root, remember it */
    cmd->local_conf->server_root = 1; 
...
/* LiJinhu added */
static const char *set_force_type(cmd_parms *cmd, void *struct_ptr, const char *arg)
{
    ap_set_string_slot_lower(cmd, struct_ptr, arg);

	apn_set_unsupport(cmd, MSG_NO_RELEVANT);
                                   ...
                                   const char *name)
{
	...
	/** for apache2nginx, find right conffile as possible */
	char* dir;
	rv = apr_cut_dir_level(&dir, conffile, cmd->pool);
	if(rv != APR_SUCCESS) {
        *recursion = 0;
        return apr_pstrcat(cmd->pool, "Invalid Include path ",
                           name, NULL);
	}
	if (!ap_is_directory(cmd->pool, dir)) {
		/** fix it, if server_root=/xxx/yyy, use /xxx */
		char *root;
		char *newfile;
		rv = apr_cut_dir_level(&root, ap_server_root, cmd->pool);
		if (rv == APR_SUCCESS){
			if ((rv = apr_filepath_merge(&newfile, root, name,
							APR_FILEPATH_TRUENAME
							| APR_FILEPATH_SECUREROOT, cmd->pool))
					== APR_SUCCESS) {
				conffile = apr_pstrdup(cmd->pool, newfile);
			}
		}
	}
...
AP_INIT_TAKE1("ForceType", set_force_type,
...
/*****************************************************************
 *
 * Core handlers for convert to nginx conf ...
 * @by Reed 2012/6/14
 */

static const char* ngx_supported_methods[] = {
    "GET",
    "HEAD",
    "POST",
    "PUT",
...
static int is_ngx_supported( const char* method )
{
    if(!method) return 0;

    int i;
    int methods = sizeof(ngx_supported_methods) / sizeof(const char*);

    for(i = 0; i < methods; i++) {
        if (strcasecmp(method, ngx_supported_methods[i]) == 0 ){
            return 1;
        }
    }
...
static const char* apn_get_methods_list(apr_pool_t *p, apr_int64_t limited)
{
    const char* methods_list = NULL;
    const char* method = NULL;
    int i;
    for (i = 0; i < METHODS; ++i) {
        if (limited & (AP_METHOD_BIT << i)) {
            method = ap_method_name_of(p, i);
            if(!method) continue;

            /** some methods is not supported by limit_except of nginx. */
            if(!is_ngx_supported(method)) continue;

            if (methods_list) {
                methods_list = apr_pstrcat(p, methods_list, " ", method, NULL);
            }else{
                methods_list = method;
            }
        }
...
static const char* apn_get_MKB_string_from_Bytes( apr_pool_t *p, size_t bytes)
{
    char *out = NULL;

    if (bytes > 1024*1024) {
        bytes = bytes / 1024*1024;
        out = apr_itoa(p, bytes);
        out = apr_pstrcat(p, out, "m", NULL);
    }else if (bytes > 1024) {
        bytes = bytes / 1024;
        out = apr_itoa(p, bytes);
        out = apr_pstrcat(p, out, "k", NULL);
    }else {
        out = apr_itoa(p, bytes);
        if (!out) return NULL;
    }
...
/*****************************************************************
 *
 * process the data from core_dir_config ...
 * @by Reed 2012/6/14
 */

/** process charset */
static void* process_charset(apn_module_t* mod, core_dir_config *dconf)
{
    unsigned add_default_charset = dconf->add_default_charset;
    const char *charset_name = dconf->add_default_charset_name;
    switch (add_default_charset){
        case ADD_DEFAULT_CHARSET_ON:
            if( charset_name ){
                mod = apn_mod_insert_sibling(mod, 
                        "charset", charset_name);
            }
            break;
        case ADD_DEFAULT_CHARSET_UNSET:
            break;
        case ADD_DEFAULT_CHARSET_OFF:
            mod = apn_mod_insert_sibling(mod, "charset", "off");
            break;
        default:
            break;
    }
...
/** process satisfy */
static void* process_satisfy( apr_pool_t* p, 
                             cmd_parms *parms,
                             apn_module_t *mod, 
                             core_dir_config *dconf)
{
    ...
    int *p_satisfy = dconf->satisfy; 
    int satisfy = SATISFY_NOSPEC;
    int i;

    if (d && p_satisfy){
        for (i = 0; i < METHODS; ++i) {
            if (parms->limited & (AP_METHOD_BIT << i)) {
                satisfy = p_satisfy[i];
                break;
            }
        }
        switch (satisfy){
            ...
            case SATISFY_ANY:
                args = "any";
                break;
            case SATISFY_NOSPEC: // default value in ngx ?
            default:
                break;
        }
        if(args){
            mod = apn_mod_insert_sibling(mod, "satisfy", args);
        }
    }
...
/** process the sendfile */
static void* process_sendfile(apn_module_t *mod, core_dir_config *dconf)
{
    unsigned int enable_sendfile = dconf->enable_sendfile;
    const char *args = NULL;
    switch (enable_sendfile){
        case ENABLE_SENDFILE_OFF  :
            args = "off";
            break;
        case ENABLE_SENDFILE_ON   :
            args = "on";
            break;
        case ENABLE_SENDFILE_UNSET:
        default:
            break;
    }
    if (args){
        mod = apn_mod_insert_sibling(mod, "sendfile", args);
    }
...
static void* process_error_page( apr_pool_t* p, 
                                cmd_parms *parms,
                                apn_module_t *mod, 
                                core_dir_config *dconf)
{
    const char *args = NULL;
    char **response_code_strings = dconf->response_code_strings;
    if (response_code_strings){
        int i=0;
        int status;
        for(i = 0; i < RESPONSE_CODES; i++) {
            args = response_code_strings[i];
            if ( args && (args != &errordocument_default)) {

                /** nginx not support message in error_page */
                if (args[0] == '"') continue;

                status = apr_atoi64(ap_status_line_of_response(p, i));
                mod = apn_mod_insert_sibling(mod, "error_page", 
                            apr_psprintf(p, "%d %s", status, args));
            }
        }
    }
...
static char * wildcard2reg(apr_pool_t *pool, const char *wild)
{
    char *special_chars = ".$^{[(|)+\\";
    char *string = "";
    string = apr_pstrcat(pool, string, "^", NULL);
    
    for ( ; wild && *wild != '\0'; wild++) {
        char wild_char[2];
        wild_char[0] = *wild;
        wild_char[1] = '\0';

        if (strstr(special_chars, wild_char) != NULL) {
            string = apr_pstrcat(pool, string, "\\", wild_char, NULL);
        } else if (*wild == '*') {
            string = apr_pstrcat(pool, string, ".*", NULL);
        } else if (*wild == '?') {
            string = apr_pstrcat(pool, string, ".", NULL);
...
/** process the sec file */
static void* process_sec_file( apr_pool_t* p, 
                              cmd_parms *parms,
                              apn_module_t *mod, 
                              core_dir_config *dconf)
{
    ...
    if (dconf->sec_file){
        ...
        for (sec_idx = 0; sec_idx < num_sec; ++sec_idx) {
            ...
            if (dir){
                /*printf("[per_dir] sec_file %s\n", entry_core->d);*/

                /** if file or filesmatch is in location */
                if(!entry_core->d_is_fnmatch && !entry_core->r ){
                    dir = wildcard2reg(p, dir);
                    /* skip the first ^ : dir + 1 */
                    dir = apr_pstrcat(p, "~ \"", dir + 1, "\"", NULL );

                } else if (entry_core->r) {
                ...
                new_node = apn_new_node( "location", 
                        apr_pstrcat(p, dir, " {", NULL ));
                if (new_node == NULL) {
                    apn_error("Failed to new a node.\n");
                    return mod;
                }
                new_node->comment = "This location comes from <Files or <FilesMatch. It's not responded to each other completely. Please check.";

                /** nginx not allow location directive in location. */
                if( d ) {
                ...
                if( rv != APR_SUCCESS ){
                    apn_error("sec_dir parse failure.\n");
                    return mod;
                }
                ...
                if ( !mod ) {
                    ...
                    mod->directives = apn_cat_node(mod->directives, new_node);
                }

                /** setting the location flag */
                apn_node_t *node = new_node->first_child;
                while(node){
...
/** process the sec limit */
static void* process_sec_limit( apr_pool_t* p, 
                               cmd_parms *parms,
                               apn_module_t *mod, 
                               core_dir_config *dconf)
{
    ...
    int sec_idx;
    int num_sec;
    const char* xmethods = NULL;
    apn_node_t * new_node = NULL;
    core_dir_config *entry_core;

    if (dconf->sec_limit && dconf->limited != -1){
        ap_conf_vector_t **sec_ent = (ap_conf_vector_t **) dconf->sec_limit->elts;
        num_sec = dconf->sec_limit->nelts;
        ...
        for (sec_idx = 0; sec_idx < num_sec; ++sec_idx) {
            ...
            new_node = apn_new_node( "limit_except", 
                    apr_pstrcat(p, xmethods, " {", NULL ));
            if (new_node == NULL) {
                apn_error("Failed to new a node.\n");
                return mod;
            }
...
/*process Options*/
static void* process_options( apr_pool_t* p, 
                             cmd_parms *parms,
                             apn_module_t *mod, 
                             core_dir_config *dconf)
{
    allow_options_t opts = dconf->opts;
    allow_options_t opts_remove = dconf->opts_remove;
    allow_options_t opts_add = dconf->opts_add;
 
    /* To convert args with (-) additional if is necessary*/ 
    if( opts_remove == OPT_NONE ) {
	if( (opts == OPT_UNSET) || (opts == (OPT_UNSET|OPT_ALL))) {
		return mod;
	}
    }
    ...
    /*args -> (+,-)Indexes*/
    if ( opts & OPT_INDEXES ){ /* Indexes */
        mod = apn_mod_insert_sibling(mod, "autoindex", on);
    }
    else if ( opts_add & OPT_INDEXES ){ /* Indexes */
        mod = apn_mod_insert_sibling(mod, "autoindex", on);
    }
    else if ( opts_remove & OPT_INDEXES ) { /* Indexes */ 
	mod = apn_mod_insert_sibling(mod, "autoindex", off);
    }   
    
    /*args -> (+,-)Includes*/
    if ( opts & (OPT_INCLUDES | OPT_INC_WITH_EXEC)){ /* Includes */
        mod = apn_mod_insert_sibling(mod, "ssi", on);
    }
    else if ( opts_add & (OPT_INCLUDES | OPT_INC_WITH_EXEC)){ /* Includes */
        mod = apn_mod_insert_sibling(mod, "ssi", on);
    }
    else if ( opts_remove & (OPT_INCLUDES | OPT_INC_WITH_EXEC)){ /* Includes */
        mod = apn_mod_insert_sibling(mod, "ssi", off);
    }


	/** whenever you change, you should 
	 *  change them in set_options .
	 */
#if 0
    if ( opts & OPT_SYM_LINKS ){ /* FollowSymLinks */
    } 

    if ( opts & OPT_SYM_OWNER ){ /* SymLinksIfOwnerMatch */
    } 

    if ( opts & OPT_EXECCGI ){ /* ExeCGI */
    } 

    if ( opts & OPT_MULTI ){ /* MultiViews */
    } 

    if ( opts & (OPT_MULTI|OPT_EXECCGI) ){  /* RunScripts */
    }
#endif
...
static void* convert_core_dir_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    core_dir_config *dconf = ap_get_module_config(v, &core_module);
    if( !dconf) return NULL;

    if(!parms){
        apn_error("parms is null.\n");
        return NULL;
    }
    ...
    /* Access control */
    /*ap_regex_t *r = dconf->r;*/
    /** the number of slashes in d */
    /*unsigned d_components= dconf->d_components;*/

    /** store to parms to process by other module in future.
     */
    parms->dir_context->output_filters = dconf->output_filters;  
    parms->dir_context->input_filters = dconf->input_filters;  
    parms->dir_context->ct_output_filters = dconf->ct_output_filters;  

    parms->dir_context->ap_auth_name = dconf->ap_auth_name;  
    parms->dir_context->ap_auth_type = dconf->ap_auth_type;  
    parms->dir_context->ap_requires = dconf->ap_requires;  
    parms->dir_context->ap_default_type = dconf->ap_default_type;
 
    const char *ap_default_type = dconf->ap_default_type;

    if( ap_default_type ){
        if (strcasecmp(ap_default_type, DEFAULT_CONTENT_TYPE) != 0 ){
            mod = 
                apn_mod_insert_sibling(mod, "default_type", ap_default_type);
        }
    }

    /** process the charset */
    mod = process_charset(mod, dconf);

    /** process the satisfy */
    mod = process_satisfy(p, parms, mod, dconf);

    /* limit on bytes in request msg body */
    apr_off_t limit_req_body = dconf->limit_req_body;
    if(limit_req_body != AP_LIMIT_REQ_BODY_UNSET){
        if ( limit_req_body == AP_DEFAULT_LIMIT_REQ_BODY ){
            limit_req_body = ((size_t)1000000);
        }
        args = apn_get_MKB_string_from_Bytes(p, limit_req_body);
        if (args){
            mod = apn_mod_insert_sibling(mod, "client_max_body_size", args);
        }
    }

    /* process the sendfile */
    mod = process_sendfile(mod, dconf);

    /** process error_page */
    mod = process_error_page(p, parms, mod, dconf);

   /** process sec_file */
    mod = process_sec_file(p, parms, mod, dconf);

    /** process sec_limit */
    mod = process_sec_limit(p, parms, mod, dconf);
    
    /*process Options*/
    mod = process_options(p, parms, mod, dconf);
...
/*****************************************************************
 *
 * process the data from core_server_config ...
 * @by Reed 2012/6/14
 */

/** process sec_dir */
static void* process_sec_dir( apr_pool_t* p, 
                             cmd_parms *parms,
                             apn_module_t *mod, 
                             core_server_config *sconf)
{
    ...
    if (sconf->sec_dir){
        ...
        for (sec_idx = 0; sec_idx < num_sec; ++sec_idx) {
            ...
            if (dir){
                ...
                parms->path = NULL; /* reset path. */

                if ( parms->alias_path ){
                    new_node->args = apr_pstrcat(p, parms->alias_path, " {", NULL);
                }
        ...
        /* traverse the parms->real_fake_rec. */
        if (parms->real_fake_rec != NULL) {
            const apr_array_header_t *tarr = apr_table_elts(parms->real_fake_rec);
            ...
            for (i = 0; i < tarr->nelts; i++) {
                ...
                new_node = NULL;
                new_node = apn_new_node( "location",
                        apr_pstrcat(p, telts[i].val, " {", NULL ));
                if (new_node == NULL) {
                    apn_error("Failed to new a node.\n");
                    return mod;
                }
                ...
                temp_mod = apn_mod_insert_sibling(temp_mod, "alias", telts[i].key);

                new_node = apn_insert_subtree(new_node, temp_mod->directives);
...
/** process sec_url */
static void* process_sec_url( apr_pool_t* p, 
                             cmd_parms *parms,
                             apn_module_t *mod, 
                             core_server_config *sconf)
{
    ...
    if (sconf->sec_url){
        ...
        for (sec_idx = 0; sec_idx < num_sec; ++sec_idx) {
            ...
            entry_core = ap_get_module_config(entry_config, &core_module);
            if (!entry_core) break;
            char *url = entry_core->d;
            if (url){
                apn_debug("[per_server] sec_url: %s\n", entry_core->d);
                if(entry_core->r) url = apr_pstrcat(p, "~ \"", url, "\"", NULL );
                new_node = apn_new_node( "location", 
                        apr_pstrcat(p, url, " {", NULL ));
                if (new_node == NULL) {
                    apn_error("Failed to new a node.\n");
                    return mod;
                }
                ...
                rv = apn_walk_dir_config(p, parms, new_node, entry_config);
                if( rv != APR_SUCCESS ){
                    apn_error("sec_url parse failure.\n");
                    return mod;
                }
...
static void* convert_core_server_config( apr_pool_t *p, 
                                     cmd_parms *parms,
                                     ap_conf_vector_t* v )
{
    core_server_config* sconf = (core_server_config*)
        ap_get_module_config(v, &core_module);
...
/* Per-server core configuration */
#ifdef GPROF
    char *gprof_dir = sconf->gprof_dir;
    /*if(gprof_dir) printf("gprof_dir=%s\n", gprof_dir );*/
#endif
    ...
    const char *ap_document_root = sconf->ap_document_root;
    if(ap_document_root) {
        /*printf("ap_document_root=%s\n", ap_document_root );*/
        mod = apn_mod_insert_sibling(mod, "root", ap_document_root);
        apn_mod_add_comment(mod, 
                "This should be changed to whatever you config to real server.");

        if(parms && parms->server)
            parms->server->ap_document_root = ap_document_root;
    }
  
    /*char *access_name = sconf->access_name;*/
    /*if(access_name) printf("access_name=%s\n", access_name );*/

    /** process sec_dir */
    mod = process_sec_dir (p, parms, mod, sconf);

    /** process sec_url */
    mod = process_sec_url (p, parms, mod, sconf);
...
AP_DECLARE_DATA module core_module = {
    ...
    convert_core_dir_config,      /* convert per dir config structure */
    convert_core_server_config    /* convert per server config structure */
};
