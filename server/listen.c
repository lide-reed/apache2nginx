                                  ...
                                  apr_port_t port, const char* proto)
{
    ...
    if ((status = apr_sockaddr_info_get(&sa, addr, APR_UNSPEC, port, 0,
        ...
        ap_log_perror(APLOG_MARK, APLOG_CRIT, status, process->pool,
                      ...
                      addr);
        if (status == 9 + APR_OS_START_EAIERR) {
           apn_warning("IPv6 not supported in this machine.\n"); 
           return NULL;
        }
    ...
    while (sa) {
        ...
        /* Li Jinhu added */
        if (strcmp(new->protocol, "https") == 0) {
            new->listen_args = apr_pstrcat(process->pool, listen_args, " ssl", NULL);
        } else {
            new->listen_args = apr_pstrcat(process->pool, listen_args, NULL);
        }
        listen_args = NULL;
