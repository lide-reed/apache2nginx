...
/*
 * Modified by Reed, avoid ap_uname2id exit, return -1 instead.
 */
                                        ...
                                        const char *arg)
{
    ...
    /** valid local user, remember it */
    if( unixd_config.user_id != -1 ){
        cmd->local_conf->user = 1;

        /** make sure it is real name */
        if (arg && arg[0] == '#'){
            unixd_config.user_name = 
                ap_uid2name(unixd_config.user_id);
        }
    }
...
/*
 * Modified by Reed, avoid ap_gname2id exit, return -1 instead.
 */
                                         ...
                                         const char *arg)
{
    ...
    unixd_config.group_name = arg; 
    unixd_config.group_id = ap_gname2id(arg);

    /** valid local group, remember it */
    if( unixd_config.group_id != -1 ){
        cmd->local_conf->group = 1;

        /** make sure it is real name */
        if (arg && arg[0] == '#'){
            unixd_config.group_name = 
                ap_gid2name(unixd_config.group_id);
        }
