...
/** added by reed, 2012/4/5 */
APR_DECLARE(apr_status_t) apr_get_realpath(char **resolved_path, 
                                           const char *filename,
                                           apr_pool_t *p)
{
    char path[APR_PATH_MAX]={0};

    if (!realpath(filename, path)) {
        return errno;
    }
    *resolved_path = apr_pstrdup(p, path);

    return APR_SUCCESS;
}

/** added by reed, 2012/4/5 */
APR_DECLARE(apr_status_t) apr_get_basedir(char **resolved_dir, 
                                           const char *filename,
                                           apr_pool_t *p)
{
    char* fullpath = NULL;
    apr_status_t rv = apr_get_realpath(&fullpath, filename, p);
    if (rv != APR_SUCCESS) 
        return rv;
    ...
    char* basedir = dirname(fullpath);
    if (!basedir) {
        return APR_EBADPATH;
    }
    *resolved_dir = apr_pstrdup(p, basedir);

    return APR_SUCCESS;
}

/** added by Reed, 2012/6/19 */
APR_DECLARE(apr_status_t) apr_cut_dir_level(char **resolved_dir, 
                                           const char *dirname,
                                           apr_pool_t *p)
{
    char* dir = apr_pstrdup(p, dirname);
	int len = strlen(dir);
	...
	if( !pos ) return APR_EBADPATH;

	*pos = '\0';

	*resolved_dir = apr_pstrdup(p, dir);
