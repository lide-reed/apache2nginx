...
/**
 * for apache2nginx, it is important to distinguish whether
 * the configuraion of apache is for local host, 
 * if it is true, the parsing of many directives, 
 * such as ServerRoot, DocumentRoot, Hostname, Listen, User, Group ...
 * is depend on the configuration of local host;
 * if it is false, maybe we can ignore some errors.
 */
typedef struct apn_local_conf_s apn_local_conf_t;
struct apn_local_conf_s {
    int user;
    int group;
    int server_root;
    int document_root;
...
/** 
 * @brief Per-directory configuration come from http_core.h
 */
typedef struct {
...
struct cmd_parms_struct {
    ...
    /** we need many dir context data come from core.c @Reed */
    context_per_dir_config *dir_context;

#define PARAM_UNDEFINE		    0x00
#define PARAM_UNSUPPORTED       0x02
#define PARAM_UNRECOGNIZED      0x04
#define PARAM_ERROR             0x08
#define PARAM_NEEDLESS_PROCESS  0x10
#define PARAM_UNKNOWN           0x20
#define PARAM_UNCONVERTED  \
	(PARAM_UNSUPPORTED | PARAM_UNRECOGNIZED | PARAM_ERROR | PARAM_NEEDLESS_PROCESS)
	/** for some directive, needless parsing, because of ... */
	int flag;
     ...
     * if it is false, maybe we can ignore some errors.
     */
    apn_local_conf_t*  local_conf; /* informations */
    int is_local_conf; /* result depend on above informations */

    /* for converting mod_alias */
    char *alias_path;
    apr_table_t *real_fake_rec;
};
...
struct module_struct {
    ...
    /** Function which used to convert per server config to nginx.
     *    @param p the pool to use for all allocations
     *  @param parms The cmd_parms to pass to all functions
     *  @param ap_conf_vector_t
     *  @modified by Reed, 2012.4.18
     */
    void *(*apn_convert_dir_config)(apr_pool_t *p, 
            cmd_parms *parms, struct ap_conf_vector_t *v);
     ...
     *  @modified by Reed, 2012.4.18
     */
    void *(*apn_convert_server_config)(apr_pool_t *p, 
            cmd_parms *parms, struct ap_conf_vector_t *v);
};
 ...
 * @return OK if no problems
 * @Author Reed, 2012/4/12
 */
AP_DECLARE(int) apn_migrate_to_nginx( apr_pool_t *p,
                                      server_rec *s,
                                      const char* ngx_confname );
/** 
 * brief parsing the virt and section_vector(if it is not null),
 * Get the directives of converted to the parent.
 * @param p The pool for general allocation
 * @param parms The cmd_parms to pass to all functions
 * @param parent Where store the directives sub tree.
 * @param section_vector the section vector, 
 *          where stored the data of dir, url and file, and etc.
 * @return parent.
 * @Author Reed, 2012/4/12
 */

AP_DECLARE(int) apn_walk_dir_config(apr_pool_t *p, 
        cmd_parms *parms, apn_node_t* parent,
        ap_conf_vector_t *section_vector);

#define MSG_NO_RELEVANT \
	"No any relevant directive is supported by Nginx"
#define MSG_NO_IMPLEMENT  \
	"The convertion is not implemented yet"
#define MSG_INHERIT_PARENT \
	"It is not converted due to its parent container directive is not converted"
#define MSG_UNRECOGNIZED_MODULE \
	"It is in an unrecognized module block, e.g. <IfMoulde unknown> ... </IfModule>"
#define MSG_UNRECOGNIZED_DIRECTIVE \
	"This directive is not been recognized"
#define MSG_NEEDLESS_PROCESS \
	"This directive have no effect according to the context"

AP_DECLARE(void) apn_set_unsupport(cmd_parms *, const char*);
